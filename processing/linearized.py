...
##### Linearize the data #####
x_p_lin = np.sqrt(x_p - 20)
y_p_lin = np.log(y_p - 20)

plt.figure(figsize=(10,6))

y_lin_upper = np.log(y_p + 2) 
y_lin_lower = np.log(y_p - 2)

y_lin_uncertainity = (y_lin_upper - y_lin_lower)/2

##### Plot the data points #####
plt.errorbar(
    x_p_lin, y_p_lin, xerr=0.05, yerr=y_lin_uncertainity,
...
x_m_lin = np.sqrt(x_m - 23)
y_m_lin = np.log(y_m - 23)
...
# Add labels
plt.title('ln(Temperature above Environment) vs. $\sqrt{(Fan Speed)}$', fontdict={'fontsize': 18})

plt.xlabel('$\sqrt{Fan Speed (RPM)}$', fontdict={'fontsize': 14})
plt.ylabel('$\ln(T-T_{\infty})$', fontdict={'fontsize': 14})
